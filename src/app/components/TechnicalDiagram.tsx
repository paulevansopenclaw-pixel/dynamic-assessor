...
export default function TechnicalDiagram({ type, imageUrl, blueprintUrl }: DiagramProps) {
  return (
    <div className="bg-white/5 p-4 rounded-2xl border border-white/10 mb-4 overflow-hidden space-y-4">
      ...
      <div className="space-y-2">
        <h4 className="text-orange-400 font-bold text-xs tracking-widest uppercase">Blue Book Requirement (Blueprint)</h4>
        {blueprintUrl ? (
          <img src={blueprintUrl} alt="Blue Book Blueprint" className="w-full h-auto rounded-lg shadow-2xl bg-white" />
        ) : (
          <p className="text-white/40 italic">Blueprint loading...</p>
        )}
      </div>
